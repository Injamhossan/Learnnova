...
export default function StudentSidebar({ collapsed, setCollapsed }: StudentSidebarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-6 h-16 border-b border-slate-100 shrink-0">
        <Link href="/student" className="flex items-center gap-2 overflow-hidden">
          <Image 
            ...
            height={32} 
            className={cn('h-8 w-auto object-contain transition-all duration-300', collapsed && 'scale-110')}
          />
          {!collapsed && (
      ...
      {/* Toggle Button in Middle (Desktop only) */}
      <button 
          onClick={() => setCollapsed(!collapsed)}
          className="hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-white border border-slate-200 rounded-full items-center justify-center text-slate-500 hover:text-slate-900 hover:shadow-md hover:border-blue-400 transition-all z-40 shadow-sm group"
      >
            ...
            <ul className="space-y-1.5">
              {section.items.map(({ href, label, icon: Icon }) => (
                <li key={href}>
                  ...
                  >
                    <Icon className={cn('w-4 h-4 shrink-0', isActive(href) ? 'text-white' : 'text-slate-400 group-hover:text-slate-600')} />
